...
public class QuestionGenerationController {
    ...
    @Operation(summary = "지정 기사 기반 문제 생성",
            description = "URL의 news_id로 지정한 기사를 대상으로 Gemini로 난이도(1~5)를 판단하고, "
                    + "오늘의 학습 문제 10개 + 게임용 문제 10개를 생성해서 DB에 저장한 뒤, 생성 결과를 응답으로도 반환한다. "
                    + "해당 news_id의 기사가 없거나 이미 문제가 생성된 기사면 에러를 반환한다.")
    @PostMapping("/preview/{news_id}")
    public BaseResponse<QuestionGenerationResult> preview(@PathVariable("news_id") Long newsId) {
        QuestionGenerationResult result = questionGenerationService.generate(newsId);
