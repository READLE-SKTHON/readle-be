...
public interface GameRoomQuestionRepository extends JpaRepository<GameRoomQuestion, Long> {
     ...
     * 같은 방/문제에 대한 동시 제출 중 하나가 커밋될 때까지 나머지는 여기서 대기한다.
     */
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("select q from GameRoomQuestion q where q.roomId = :roomId and q.displayOrder = :displayOrder")
    Optional<GameRoomQuestion> findByRoomIdAndDisplayOrderForUpdate(
            @Param("roomId") Long roomId, @Param("displayOrder") Integer displayOrder);
}
